function [y,out]=proc_IMCRA(fin,fout)
...
% Read input data
[~,Fs]=wavread(fin,1);  % read size of input data, Fs and NBITS
N=wavread(fin,'size');
N = N(1);
...
for l=1:Nframes
    if l==1
        ...
        y=fread(finID,M,'int16');
    else
        %[y0,Fs,~,fidx]=readwav(fidx,'rf',Mno);
        y0=fread(finID,Mno,'int16');
    ...
    if l==1
        ...
        fwrite(foutID,[initfin; out(1:Mno)],'int16');
    else
        fwrite(foutID,out(1:Mno),'int16');   % write to output file
    end
...
% close(waitHandle)
fwrite(foutID,out(1:M-Mno),'int16');  % write to output file the last output samples
