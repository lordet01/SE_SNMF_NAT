...
p.NOISE_NUM = 1;
p.NOISE_RANK(1) = 1;
p.train_Exemplar = 1; %train with SNMF if 0
...
p.m_a = 40; %No. of stacked block for basis adaptation
p.overlap_m_a = 0.5; %No. of stacked block for basis adaptation
 
%Block sparsity options
p.blk_sparse = 0; %block sparsity switch
p.P_len_k = 50; % vertical (frequency bin) size of Block for local sparsity calculation
p.P_len_l = 3; % horizental (time frame index) size of Block for local sparsity calculation
p.kappa = 0.8;
p.nu = 1.2;
p.alpha_p = 0.6; %DD smoothing factor for P
...
p.sparsity = 5;
p.max_iter = 50; % Stopping criteria
p.conv_eps = 1e-3; 
...
p.ENHANCE_METHOD = 'Wiener'; %['Wiener', 'MMSE']
%2.1) Parameters of "Decision-Directed" a Priori SNR Estimate
p.alpha_eta=0.95;	% Recursive averaging parameter
...
p.beta = 2.0; %Bias compensation factor in the noise estimator
...
OUTname = [Testname,'_',p.NMF_algorithm,'_A',num2str(p.adapt_train_N),'_M',num2str(p.MDI_est_noise),'_r',num2str(p.R_x),'_p',num2str(p.pow), ...
           '_',p.ENHANCE_METHOD,'_P',num2str(p.blk_sparse),'_Exemplar'];