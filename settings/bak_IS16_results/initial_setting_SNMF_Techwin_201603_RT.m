...
p.m_a = 16; %No. of stacked block for basis adaptation
p.overlap_m_a = 0.5; %No. of stacked block for basis adaptation
p.Ar_up = 0.8; %Define Ax and Ad ratio for noise dictionary update (Lower: Update frequently, Higher: Update rarely)
...
p.blk_gap = 5; %Blk_gap for complexity issue (1 for ideal), Odd only!
...
p.sparsity = 5;
p.max_iter = 15; % Stopping criteria
p.conv_eps = 1e-3; 
...
p.beta = 1.0; %Bias compensation factor in the noise estimator
...
OUTname = [Testname,'_',p.NMF_algorithm,'_A',num2str(p.adapt_train_N),'_M',num2str(p.MDI_est_noise),'_r',num2str(p.R_x),'_p',num2str(p.pow), ...
           '_',p.ENHANCE_METHOD,'_P',num2str(p.blk_sparse),'_Proposed_Techwin_201603'];