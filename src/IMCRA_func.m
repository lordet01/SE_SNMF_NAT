function [N]=IMCRA_func(Y, g, p)
% IMCRA noise estimator
% ***************************************************************@
% Inputs:
%    Y,  spectral magnitudes of input frame
% Output:
%    N,  spectral magnitudes of estimated noise frame by IMCRA
% Usage:
%    [N]=IMCRA_func(Y);
%
% Copyright (c) 2003. Prof Israel Cohen.
% All rights reserved. 
% Modified by Kwang Myung Jeon (2016).
% ***************************************************************@



% 1) Parameters of Short Time Fourier Analysis:
Fs_ref=p.fs;		% 1.1) Reference Sampling frequency
M_ref=p.fftlength;		% 1.2) FFT length
l = g.cnt;          %current frame idx
...
% You may choose zero_thres=0, but then the program  handles samples which are identically zero (and not 밻psilon values).
...
N=[zeros(3,1); G_inv(4:M21-1).*Y(4:M21-1); 0];
