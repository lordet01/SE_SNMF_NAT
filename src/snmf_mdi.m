function [v_MDI, h, objective] = snmf_mdi(v, Dm, p)
...
lambda = max(w * h, flr);
last_cost = Inf;
v= max(v.*Dm, flr); %Initial condition of masked input
...
Nt = sum(v.*Dm,1) ./ max(sum(v_est.*Dm,1), flr);
v_impute = (repmat(Nt,[m 1]) .* v_est) .* ~Dm;
v_MDI = max(v .* Dm + v_impute, flr);
